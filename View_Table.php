...
# Эта версия с возможными линками в столбцах                                       #
####################################################################################
#
...
#
$Table_view =  new HTML_Table($headers , $heard_color = 'red'); 
#
...
#
### Но можно сюда заранее поместить все $Table_Links, но тогда отдельно надо от $Table_Headers !!!
#  
foreach ($QueryResult as $row => $Arr_acc)
      {
       ...
       foreach ($headers_db as $element) 
               {
          if ($Table_Links[$FormType][$element]){    // если нужны линки в столбцах
          $KeyValue    = $Arr_acc[$element];          // если по первому ключу, но может быть и другое
          $Link_Begin = "<a href=FirstKey_Search.php?FormType=8_ocenky&KeyValue=$KeyValue&Print=1>";
          $Link_End   = '</a>';
          $Arr_acc[$headers_db[$i]] = $Link_Begin.$Arr_acc[$headers_db[$i]].$Link_End;
          }   
